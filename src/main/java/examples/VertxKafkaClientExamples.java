...
public class VertxKafkaClientExamples {
  ...
  public void createProducer(Vertx vertx) {
    // creating the producer using map and class types for key and value
  ...
  public void exampleConsumerPartitionsNotifs(KafkaConsumer<String, String> consumer) {
    ...
    consumer.partitionsLostHandler(topicPartitions -> {
      System.out.println("Partitions lost");
      for (TopicPartition topicPartition : topicPartitions) {
        System.out.println(topicPartition.getTopic() + " " + topicPartition.getPartition());
      }
    });
  ...
  public void exampleUnsubscribeWithCallback(KafkaConsumer<String, String> consumer) {
    ...
    consumer
        .unsubscribe()
        .onSuccess(v -> System.out.println("Consumer unsubscribed"));
  }
  ...
  public void exampleConsumerAssignPartition(KafkaConsumer<String, String> consumer) {
    ...
    consumer
        ...
        // After the assignment is completed, get the assigned partitions to this
        // consumer
  ...
  public void exampleConsumerListTopics(KafkaConsumer<String, String> consumer) {
    ...
    consumer
        .listTopics()
        .onSuccess(partitionsTopicMap -> partitionsTopicMap.forEach((topic, partitions) -> {
  ...
  public void exampleConsumerWithPoll(Vertx vertx, KafkaConsumer<String, String> consumer) {
    ...
    consumer
        ...
        .onSuccess(v -> {
          ...
          vertx.setPeriodic(1000, timerId -> consumer
  ...
  /**
   * Example to demonstrate how one can use the new beginningOffsets API
   * (introduced with Kafka
  ...
  public void exampleConsumerEndOffsets(KafkaConsumer<String, String> consumer) {
    ...
    consumer
        ...
        .onSuccess(endOffset -> System.out.println(
            "End offset for topic=" + topicPartition.getTopic() + ", partition=" +
                topicPartition.getPartition() + ", endOffset=" + endOffset));
  }

  /**
   * Example to demonstrate how one can use the new offsetsForTimes API
   * (introduced with Kafka
   * 0.10.1.1) to look up an offset by timestamp, i.e. search parameter is an
   * epoch timestamp and
   * the call returns the lowest offset with ingestion timestamp >= given
   * timestamp.
   * 
   * @param consumer Consumer to be used
   */
  public void exampleConsumerOffsetsForTimes(KafkaConsumer<String, String> consumer) {
    ...
    consumer
        ...
        .onSuccess(results -> results.forEach((topic, offset) -> System.out.println(
    ...
    // Convenience method for single-partition lookup
    consumer.offsetsForTimes(topicPartition, timestamp).onSuccess(offsetAndTimestamp -> System.out.println(
  ...
  public void exampleProducerWrite(KafkaProducer<String, String> producer) {
    for (int i = 0; i < 5; i++) {
      ...
      // only topic and message value are specified, round robin on destination
      // partitions
  ...
  public void exampleProducerWriteWithAck(KafkaProducer<String, String> producer) {
    for (int i = 0; i < 5; i++) {
      ...
      producer.send(record).onSuccess(recordMetadata -> System.out.println(
  ...
  public void exampleProducerWriteWithSpecificKey(KafkaProducer<String, String> producer) {
    for (int i = 0; i < 10; i++) {
      ...
      // a key is specified, all messages with same key will be sent to the same
      // partition
  ...
  public void exampleUsingVertxDeserializers2(Vertx vertx) {
    ...
    KafkaConsumer<JsonObject, JsonObject> jsonObjectConsumer = KafkaConsumer.create(vertx, config, JsonObject.class,
        JsonObject.class);

    // Creating a consumer able to deserialize json arrays
    KafkaConsumer<JsonArray, JsonArray> jsonArrayConsumer = KafkaConsumer.create(vertx, config, JsonArray.class,
        JsonArray.class);
  }
  ...
  public void exampleUsingVertxSerializers2(Vertx vertx) {
    ...
    KafkaProducer<JsonObject, JsonObject> jsonObjectProducer = KafkaProducer.create(vertx, config, JsonObject.class,
        JsonObject.class);

    // Creating a producer able to serialize to json arrays
    KafkaProducer<JsonArray, JsonArray> jsonArrayProducer = KafkaProducer.create(vertx, config, JsonArray.class,
        JsonArray.class);
  }
}
