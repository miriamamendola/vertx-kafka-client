 ...
 * <p>
 * You receive Kafka records by providing a
 * {@link KafkaConsumer#handler(Handler)}. As messages arrive the handler
 * will be called with the records.
 * <p>
 * The {@link #pause()} and {@link #resume()} provides global control over
 * reading the records from the consumer.
 * <p>
 * The {@link #pause(Set)} and {@link #resume(Set)} provides finer grained
 * control over reading records
...
public interface KafkaConsumer<K, V> extends ReadStream<KafkaConsumerRecord<K, V>> {
  ...
  static <K, V> KafkaConsumer<K, V> create(Vertx vertx, Map<String, String> config,
    ...
    KafkaReadStream<K, V> stream = KafkaReadStream.create(vertx, new HashMap<>(config), keyDeserializer,
        valueDeserializer);
    return new KafkaConsumerImpl<>(stream).registerCloseHook();
  }
   ...
   *
   * <ul>
   * <i>If the stream is in <i>flowing</i> mode will return
   * {@link Long#MAX_VALUE}.</i>
   * <li>If the stream is in <i>fetch</i> mode, will return the current number of
   * elements still to be delivered or 0 if paused.</li>
   * </ul>
   *
  ...
  /**
   * Set the handler called when topic partitions are lost to the consumer
   *
   * @param handler handler called on lost topic partitions
   * @return current KafkaConsumer instance
   */
  @Fluent
  KafkaConsumer<K, V> partitionsLostHandler(Handler<Set<TopicPartition>> handler);
  ...
  /**
   * Get the offset of the next record that will be fetched (if a record with that
   * offset exists).
   *
  ...
  /**
   * Look up the offsets for the given partitions by timestamp. Note: the result
   * might be empty in case
   * for the given timestamp no offset can be found -- e.g., when the timestamp
   * refers to the future
   * 
   * @param topicPartitionTimestamps A map with pairs of (TopicPartition,
   *                                 Timestamp).
   * @return a future notified on operation completed
   */
  @GenIgnore
  ...
  /**
   * Look up the offset for the given partition by timestamp. Note: the result
   * might be null in case
   * for the given timestamp no offset can be found -- e.g., when the timestamp
   * refers to the future
   * 
  ...
  /**
   * Get the last offset for the given partition. The last offset of a partition
   * is the offset
   * of the upcoming message, i.e. the offset of the last available message + 1.
   * 
  ...
  /**
   * Sets the poll timeout for the underlying native Kafka Consumer. Defaults to
   * 1000ms.
   * Setting timeout to a lower value results in a more 'responsive' client,
   * because it will block for a shorter period
   * if no data is available in the assigned partition and therefore allows
   * subsequent actions to be executed with a shorter
   * delay. At the same time, the client will poll more frequently and thus will
   * potentially create a higher load on the Kafka Broker.
   *
   * @param timeout The time, spent waiting in poll if data is not available in
   *                the buffer.
   *                If 0, returns immediately with any records that are available
   *                currently in the native Kafka consumer's buffer,
